...
exports.login = (req, res) => {
  ...
  db.query(query, [username, password], (error, results) => {
    ...
    if (results.length > 0) {
      // If a user is found, generate a token (here we use a mock token for demonstration)
      const token = "mock-token-123"; // In production, generate a secure JWT or similar token
